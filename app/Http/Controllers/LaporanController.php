...
class LaporanController extends Controller
{
    ...
    public function cetakpresensi(Request $request)
    {
        ...
        $q_presensi->select(
            ...
            'penambah',
            'pengurang',
            'potongan_pinjaman.total_potongan_pinjaman',
            'potongan_pinjaman.jumlah_cicilan',
            ...$select_field_tunjangan
        );
        ...
        $q_presensi->leftJoinSub($potongan_pinjaman, 'potongan_pinjaman', function ($join) {
            $join->on('karyawan.nik', '=', 'potongan_pinjaman.nik');
        });
        ...
        if (!empty($request->nik) && $request->format_laporan == 1) {
            ...
            $laporan_presensi = $presensi->groupBy('nik')->map(function ($rows) use ($jenis_tunjangan) {
                $data = [
                    ...
                    'total_potongan_pinjaman' => $rows->first()->total_potongan_pinjaman ?? 0,
                    'jumlah_cicilan' => $rows->first()->jumlah_cicilan ?? 0,
