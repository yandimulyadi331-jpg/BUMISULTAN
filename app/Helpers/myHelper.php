...
{
    if (empty($file)) {
        return url('/assets/img/avatars/No_Image_Available.jpg');
    }
    
    // Use file.php for reliable access
    $url = url('/file.php?path=yayasan_masar/' . urlencode($file) . '&v=' . time());
    return $url;
}

function getBannerInformasi($path)
{
    if (empty($path)) {
        return url('/assets/img/no-image.jpg');
    }
    
    // Use file.php for reliable access to informasi/banners folder
    $url = url('/file.php?path=' . urlencode($path) . '&v=' . time());
    return $url;
}

function getFotoWajah($nik, $fileName)
{
    if (empty($nik) || empty($fileName)) {
        return url('/assets/img/avatars/No_Image_Available.jpg');
    }
    
    // Build path: uploads/facerecognition/nik-nama/filename
    $path = 'uploads/facerecognition/' . $nik . '/' . $fileName;
...
function getLogoPerusahaan($filename)
{
    if (empty($filename)) {
        return asset('assets/img/logos/default-logo.png');
    }
    
    // Use file.php for reliable access
    $url = url('/file.php?path=logo/' . urlencode($filename) . '&v=' . time());
    return $url;
}

function getPwaIcon($sizeName)
{
    $iconPath = 'assets/img/icons/pwa/icon-' . $sizeName . '.png';
    
    if (file_exists(public_path($iconPath))) {
        // Cache busting dengan filemtime
        $filePath = public_path($iconPath);
        $mtime = file_exists($filePath) ? filemtime($filePath) : time();
        return asset($iconPath . '?v=' . $mtime);
    }
    
    return asset('assets/img/icons/pwa/icon-192x192.png');
}
