@extends('layouts.app')
@section('titlepage', 'Data Jamaah')

@section('content')
@section('navigasi')
    <span class="text-muted fw-light">Manajemen Yayasan /</span> Data Jamaah
@endsection
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0"><i class="ti ti-users me-2"></i>Data Jamaah</h5>
                <div class="d-flex gap-2">
                    <a href="{{ route('majlistaklim.jamaah.create') }}" class="btn btn-primary btn-sm">
                    ...
                    <a href="{{ route('majlistaklim.jamaah.export') }}" class="btn btn-info btn-sm">
                        <i class="ti ti-file-export me-1"></i> Export Excel
                    </a>
                    <a href="{{ route('majlistaklim.jamaah.downloadTemplate') }}" class="btn btn-secondary btn-sm">
...
<!-- Modal Get Data Mesin Fingerspot -->
...
<div class="modal fade" id="modalImport" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('majlistaklim.jamaah.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <div class="form-text">
                            ...
                            <a href="{{ route('majlistaklim.jamaah.downloadTemplate') }}" class="text-primary">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">
                            ...
                            <a href="{{ route('majlistaklim.jamaah.kehadiran.template') }}" class="text-warning">
...
<script>
    $(document).ready(function() {
        ...
        let table = $('#tableJamaah').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('majlistaklim.jamaah.index') }}",
                data: function(d) {
            ...
            columns: [
                ...
                {data: 'badge_kehadiran', name: 'jumlah_kehadiran', orderable: true},
        ...
        $(document).on('change', '.toggle-umroh', function() {
            ...
            $.ajax({
                url: "{{ url('majlistaklim/jamaah') }}/" + id + "/toggle-umroh",
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
        ...
        $(document).on('click', '.delete-btn', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('majlistaklim/jamaah') }}/" + id,
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
        ...
        $('#formImportKehadiran').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('majlistaklim.jamaah.kehadiran.import') }}",
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        // ========================================
        // FINGERSPOT GET DATA MESIN HANDLER
        // ========================================
        $(document).on('click', '.btngetDatamesin', function(e) {
            ...
            // Open modal
            $('#modal').modal('show');
            $('#loadgetdatamesin').html('<div class="text-center py-5"><i class="ti ti-loader rotating" style="font-size: 48px;"></i><p class="mt-3">Loading data dari mesin Fingerspot Cloud...</p></div>');
            
            // Tampilkan form input tanggal dulu
            Swal.fire({
                title: 'Pilih Tanggal Absensi',
                html: '<input type="date" id="tanggalAbsensi" class="form-control" value="' + new Date().toISOString().split('T')[0] + '">',
                icon: 'question',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    let tanggal = result.value;
                    
                    // AJAX request ke controller
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('majlistaklim.jamaah.getdatamesin') }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                            pin_fingerprint: pin,
                            tanggal: tanggal
                        ...
                        success: function(respond) {
                            $('#loadgetdatamesin').html(respond);
                        },
                        error: function(xhr) {
                            $('#loadgetdatamesin').html(
                                '<div class="alert alert-danger">' +
                                '<i class="ti ti-alert-circle me-2"></i>' +
                                '<strong>Error!</strong> ' + 
                                (xhr.responseJSON?.message || 'Gagal mengambil data dari server') +
                                '</div>'
                            );
                        }
                    });
